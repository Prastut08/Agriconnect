...
import { ShoppingBag, Eye, EyeOff, AlertCircle, ArrowLeft } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = isLogin
        ...
        : await signup(formData.name, formData.email, formData.password, formData.phone, 'customer');
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-10 bg-white">
        <div className="w-full max-w-md">
          ...
          <div className="lg:hidden flex items-center gap-2 mb-8">
            ...
            <span className="font-bold text-blue-800 text-lg">AgriConnect</span>
          </div>
          ...
          <div className="mb-8">
            <div className="inline-flex items-center gap-2 bg-blue-50 border border-blue-200 rounded-full px-3 py-1 mb-4">
              ...
              <span className="text-xs font-semibold text-blue-700">Customer Portal</span>
            </div>
            <h2 className="text-3xl font-black text-gray-900 mb-1">
              {isLogin ? 'Welcome back' : 'Create account'}
            </h2>
            <p className="text-gray-500 text-sm">
              {isLogin ? 'Sign in to your customer account' : 'Start buying fresh from local farmers'}
            </p>
          </div>
          ...
          <div className="flex bg-gray-100 rounded-xl p-1 mb-6">
            <button
              ...
              className={`flex-1 py-2 text-sm font-semibold rounded-lg transition-all ${isLogin ? 'bg-white text-blue-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
            >
              Sign In
            </button>
            <button
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {!isLogin && (
              <div>
                ...
                <input
                  ...
                  required
                  className="w-full px-4 py-3 rounded-xl border border-gray-200 text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 transition-all text-sm"
                />
              </div>
            )}
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  required
                  className="w-full px-4 py-3 pr-11 rounded-xl border border-gray-200 text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 transition-all text-sm"
                />
                <button
                  type="button"
            ...
            <button
              type="submit"
              disabled={submitting}
              className="w-full py-3.5 bg-blue-700 hover:bg-blue-800 text-white font-bold rounded-xl transition-all shadow-sm hover:shadow-md disabled:opacity-60 disabled:cursor-not-allowed text-sm mt-2"
            >
              {submitting ? (
                ...
                isLogin ? 'Sign In to Customer Portal' : 'Create Customer Account'
              )}
            </button>
          </form>
          ...
          <p className="text-center text-xs text-gray-400 mt-4">
            Are you a farmer?{' '}
            <Link to="/auth/farmer" className="text-emerald-600 hover:underline font-medium">
              Farmer Portal →
            </Link>
          </p>
        </div>
      ...
      <div className="hidden lg:flex lg:w-1/2 flex-col justify-between p-12 relative overflow-hidden bg-gradient-to-br from-blue-950 via-blue-900 to-indigo-800">
        ...
        <div className="relative z-10 text-right">
          <p className="text-blue-200/50 text-xs">
            Connecting 200,000+ customers with local farmers
          </p>
        </div>
      </div>
    </div>
